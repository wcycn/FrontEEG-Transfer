#!/usr/bin/env python3
"""Prepare Fp1/Fp2 raw MATB windows for all 26 valid COG-BCI subjects."""
...
def main() -> None:
    ...
    script = Path(__file__).with_name("prepare_cog_matb_raw_subject.py")
    archives = [
        archive
    ...
    if len(archives) != 26:
        raise ValueError(f"expected 26 valid archives, found {len(archives)}")
    reports = []
